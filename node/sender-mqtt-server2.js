...
function connectMQTT() {
    ...
    mqttClient.on('connect', () => {
        console.log(`✅ Conectado a MQTT Server: ${process.env.MQTT_SERVER}:${process.env.MQTT_PORT}`);
...
async function getDataFromDatabase() {
    try {
        const [rows] = await dbConnection.execute('SELECT * FROM mqtt_send_server2 ORDER BY id ASC LIMIT 100');
        return rows;
    } catch (error) {
...
async function deleteProcessedEntries(ids) {
    try {
        ...
        await dbConnection.execute(`DELETE FROM mqtt_send_server2 WHERE id IN (${placeholders})`, integerIds);
