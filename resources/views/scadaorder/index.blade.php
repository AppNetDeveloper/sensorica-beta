...
@section('title', 'SCADA Order Kanban Control')
...
@section('breadcrumb')
    <ul class="breadcrumb">
        ...
        <li class="breadcrumb-item">{{ __('SCADA Order Kanban Control') }}</li>
    </ul>
@endsection
...
@push('scripts')
    ...
    <script>
        ...
        async function loadKanbanData() {
            try {
                const response = await fetch(`/api/scada-orders?token=${token}`);
        ...
        async function showCardMenu(orderId) {
            try {
                const response = await fetch(`/api/scada-orders/${orderId}`);
