            ...
            <div class="card border-0 shadow" style="width: 100%;">
                <div class="card-header bg-primary text-white">
                    <div class="d-flex justify-content-between align-items-center">
                        <h5 class="mb-0 text-white">@lang('List of Articles') - {{ $articleFamily->name }}</h5>
                        @can('article-create')
                        <a href="{{ route('article-families.articles.create', $articleFamily) }}" class="btn btn-light btn-sm">
                            <i class="fas fa-plus"></i> @lang('New Article')
                        </a>
                        @endcan
                    </div>
                </div>
                <div class="card-body">
                    ...
                    <div class="table-responsive" style="width: 100%; margin: 0 auto;">
                        <table id="articles-table" class="table table-striped table-hover" style="width: 100%;">
                            ...
                            <tbody>
                                @forelse($articles as $index => $article)
                                    <tr>
                                        ...
                                        <td>
                                            @can('article-show')
                                            <a href="{{ route('article-families.articles.show', [$articleFamily, $article]) }}" class="btn btn-sm btn-info" title="@lang('View')">
                                                <i class="fas fa-eye"></i>
                                            </a>
                                            @endcan
                                            
                                            @can('article-edit')
                                            <a href="{{ route('article-families.articles.edit', [$articleFamily, $article]) }}" class="btn btn-sm btn-warning" title="@lang('Edit')">
                                                <i class="fas fa-edit"></i>
                                            </a>
                                            @endcan
                                            
                                            @can('article-delete')
                                            <form action="{{ route('article-families.articles.destroy', [$articleFamily, $article]) }}" method="POST" style="display: inline-block;">
                                                @csrf
                                                @method('DELETE')
                                                <button type="submit" class="btn btn-sm btn-danger" title="@lang('Delete')" onclick="return confirm('@lang('Are you sure you want to delete this article?')')">
...
@push('styles')
    ...
    <style>
        ...
        #articles-table_wrapper .dt-buttons {
            margin-bottom: 10px;
        }
        ...
        $(document).ready(function() {
            const table = $('#articles-table').DataTable({
                ...
                initComplete: function() {
                    ...
                    $('#articles-table_wrapper').addClass('pb-3');
                    $('#articles-table_length label').addClass('font-weight-normal');
                    $('#articles-table_filter label').addClass('font-weight-normal');
                    $('#articles-table_paginate').addClass('mt-3');
