...
@section('title', __('Nuevo activo') . ' - ' . $customer->name)
@section('page-title', __('Nuevo activo'))
...
<div class="mb-4">
  <ul class="breadcrumb">
    ...
    <li class="breadcrumb-item active">{{ __('Nuevo activo') }}</li>
  </ul>
</div>
@endsection
...
<div class="card">
  <div class="card-header">
    <h5 class="mb-0">{{ __('Crear activo') }}</h5>
  </div>
  <div class="card-body">
    <form action="{{ route('customers.assets.store', $customer) }}" method="POST">
      @csrf
      <div class="row g-3">
        <div class="col-md-6">
          ...
          <select name="asset_category_id" id="asset_category_id" class="form-select @error('asset_category_id') is-invalid @enderror" required>
            ...
            @foreach($categories as $id => $label)
              <option value="{{ $id }}" @selected(old('asset_category_id') == $id)>{{ $label }}</option>
            @endforeach
          </select>
        ...
        <div class="col-md-6">
          ...
          <select name="asset_cost_center_id" id="asset_cost_center_id" class="form-select @error('asset_cost_center_id') is-invalid @enderror">
            ...
            @foreach($costCenters as $id => $label)
              <option value="{{ $id }}" @selected(old('asset_cost_center_id') == $id)>{{ $label }}</option>
            @endforeach
          </select>
        ...
        <div class="col-md-6">
          ...
          <select name="asset_location_id" id="asset_location_id" class="form-select @error('asset_location_id') is-invalid @enderror">
            ...
            @foreach($locations as $id => $label)
              <option value="{{ $id }}" @selected(old('asset_location_id') == $id)>{{ $label }}</option>
            @endforeach
          </select>
        ...
        <div class="col-md-6">
          ...
          <select name="vendor_supplier_id" id="vendor_supplier_id" class="form-select @error('vendor_supplier_id') is-invalid @enderror">
            ...
            @foreach($suppliers as $id => $label)
              <option value="{{ $id }}" @selected(old('vendor_supplier_id') == $id)>{{ $label }}</option>
            @endforeach
          </select>
        ...
        <div class="col-md-6">
          ...
          <input type="text" name="article_code" id="article_code" class="form-control @error('article_code') is-invalid @enderror" value="{{ old('article_code') }}" required>
          @error('article_code')
            <div class="invalid-feedback">{{ $message }}</div>
          @enderror
        </div>
        ...
        <div class="col-md-4">
          ...
          <select name="status" id="status" class="form-select @error('status') is-invalid @enderror" required>
            @foreach($statuses as $status)
              <option value="{{ $status }}" @selected(old('status', 'active') == $status)>{{ __($status) }}</option>
            @endforeach
          </select>
          @error('status')
            <div class="invalid-feedback">{{ $message }}</div>
        ...
        <div class="col-md-4">
          ...
          <input type="date" name="acquired_at" id="acquired_at" class="form-control @error('acquired_at') is-invalid @enderror" value="{{ old('acquired_at') }}">
          @error('acquired_at')
            <div class="invalid-feedback">{{ $message }}</div>
          @enderror
        </div>
        <div class="col-md-4">
          ...
          <div class="form-check form-switch">
            <input class="form-check-input" type="checkbox" id="has_rfid_tag" name="has_rfid_tag" value="1" @checked(old('has_rfid_tag'))>
            <label class="form-check-label" for="has_rfid_tag">{{ __('Este activo tendrá etiqueta RFID') }}</label>
          </div>
        </div>
        <div class="col-md-6" id="rfid_tid_wrapper">
          <label for="rfid_tid" class="form-label">{{ __('RFID TID (único)') }}</label>
          <div class="input-group">
            <input type="text" name="rfid_tid" id="rfid_tid" class="form-control @error('rfid_tid') is-invalid @enderror" value="{{ old('rfid_tid') }}">
            <button type="button" class="btn btn-outline-secondary" id="generateTid">{{ __('Generar') }}</button>
          </div>
          @error('rfid_tid')
            <div class="invalid-feedback">{{ $message }}</div>
          @enderror
        ...
        <div class="col-12">
          ...
          <textarea name="attributes" id="attributes" rows="3" class="form-control @error('attributes') is-invalid @enderror" placeholder='{"color":"Rojo","grosor":"16mm"}'>{{ old('attributes') }}</textarea>
          @error('attributes')
            <div class="invalid-feedback">{{ $message }}</div>
          @enderror
        </div>
      ...
      <div class="d-flex justify-content-end gap-2 mt-4">
        ...
        <button type="submit" class="btn btn-primary">{{ __('Guardar activo') }}</button>
      </div>
    </form>
  </div>
</div>
...
<script>
  ...
  document.getElementById('generateTid').addEventListener('click', function(){
