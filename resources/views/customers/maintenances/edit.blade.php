    ...
    <form method="POST" action="{{ route('customers.maintenances.update', [$customer->id, $maintenance->id]) }}">
      ...
      <div class="mb-3">
        ...
        <div class="form-check form-check-inline">
          <input class="form-check-input" type="radio" name="production_line_stop" id="pls_yes" value="1" {{ (string)old('production_line_stop', (string)$maintenance->production_line_stop) === '1' ? 'checked' : '' }}>
          <label class="form-check-label" for="pls_yes">{{ __('Yes, stop the machine (blocking)') }}</label>
        </div>
        <div class="form-check form-check-inline">
          <input class="form-check-input" type="radio" name="production_line_stop" id="pls_no" value="0" {{ (string)old('production_line_stop', (string)$maintenance->production_line_stop) === '0' ? 'checked' : '' }}>
