...
<head>
    ...
    <title>Ruta Completa - {{ $routeName->name }} - {{ $dayDate->format('d/m/Y') }}</title>
    <style>
        * {
            margin: 0;
            padding: 0;
...
<body>
    <div class="header">
        ...
        <div class="header-info">
            <div style="font-weight: 600; font-size: 16px;">{{ $dayDate->format('d/m/Y') }}</div>
            <div style="color: #6c757d;">{{ $dayDate->locale('es')->isoFormat('dddd') }}</div>
        </div>
    </div>

    <div class="vehicle-info">
        <h2>🛣️ {{ $routeName->name }}</h2>
        <div class="info-grid">
            <div class="info-item">
                <span class="info-label">Vehículos Asignados</span>
                <span class="info-value">{{ $vehicleAssignments->count() }}</span>
            </div>
            <div class="info-item">
                <span class="info-label">Total Clientes</span>
                <span class="info-value">{{ $clientAssignments->count() }}</span>
            </div>
            <div class="info-item">
                <span class="info-label">Total Pedidos</span>
                <span class="info-value">{{ $clientAssignments->sum(function($ca) { return $ca->orderAssignments->count(); }) }}</span>
            </div>
        </div>
    </div>
        ...
        @if($clientAssignments->count() > 0)
            ...
            @foreach($clientAssignments as $index => $clientAssignment)
                @php
                    ...
                    // Mostrar cabecera de vehículo si cambia
                    $showVehicleHeader = ($currentVehicleId !== $clientAssignment->fleet_vehicle_id);
                    $currentVehicleId = $clientAssignment->fleet_vehicle_id;
                @endphp
                
                @if($showVehicleHeader)
                    <div style="background: #0d6efd; color: white; padding: 10px 15px; border-radius: 6px; margin-bottom: 15px; margin-top: {{ $index > 0 ? '30px' : '0' }};">
                        <strong>🚚 {{ $clientAssignment->fleetVehicle->plate }}</strong>
                        @if($clientAssignment->fleetVehicle->vehicle_type)
                            <span style="opacity: 0.9; margin-left: 10px;">({{ $clientAssignment->fleetVehicle->vehicle_type }})</span>
                        @endif
                    </div>
                @endif
