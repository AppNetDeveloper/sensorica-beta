...
@section('title', __('Nuevo pedido a proveedor') . ' - ' . $customer->name)
@section('page-title', __('Nuevo pedido a proveedor'))
...
<div class="card">
  <div class="card-header">
    <h5 class="mb-0">{{ __('Datos del pedido') }}</h5>
  </div>
  <div class="card-body">
    <form action="{{ route('customers.vendor-orders.store', $customer) }}" method="POST">
      @csrf
      <div class="row g-3">
        <div class="col-md-4">
        ...
        <div class="col-md-4">
          ...
          <input type="text" name="reference" value="{{ old('reference') }}" class="form-control @error('reference') is-invalid @enderror" required>
          @error('reference')
            <div class="invalid-feedback">{{ $message }}</div>
          @enderror
        </div>
        <div class="col-md-4">
          ...
          <select name="status" class="form-select @error('status') is-invalid @enderror" required>
            @foreach($statuses as $status)
              <option value="{{ $status }}" {{ old('status', 'draft') === $status ? 'selected' : '' }}>{{ __($status) }}</option>
            @endforeach
          </select>
          @error('status')
            <div class="invalid-feedback">{{ $message }}</div>
        ...
        <div class="col-md-4">
          ...
          <input type="text" name="currency" value="{{ old('currency', 'EUR') }}" class="form-control @error('currency') is-invalid @enderror" required>
          @error('currency')
            <div class="invalid-feedback">{{ $message }}</div>
          @enderror
        </div>
        <div class="col-md-4">
          ...
          <input type="date" name="expected_at" value="{{ old('expected_at') }}" class="form-control @error('expected_at') is-invalid @enderror">
          @error('expected_at')
            <div class="invalid-feedback">{{ $message }}</div>
          @enderror
        </div>
      ...
      <div id="order-lines">
        @php($oldLines = old('lines', [['vendor_item_id' => null, 'description' => '', 'quantity_ordered' => 1, 'unit_price' => null, 'tax_rate' => null]]))
      ...
      <div class="mt-4 d-flex justify-content-between">
        <a href="{{ route('customers.vendor-orders.index', $customer) }}" class="btn btn-outline-secondary">{{ __('Cancelar') }}</a>
        <button type="submit" class="btn btn-primary">{{ __('Guardar pedido') }}</button>
      </div>
    </form>
  </div>
</div>
