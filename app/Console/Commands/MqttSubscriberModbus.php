...
class MqttSubscriberModbus extends Command
{
    ...
    {
        // Ejecutar en un bucle infinito para la actualización continua
        while (true) {
            $this->processModbusData();
            sleep(0.1); // Espera antes de la próxima consulta
        }
    }

    public function processModbusData()
    {
        // Agrupar configuraciones por URL
        $groupedConfigs = Modbus::whereNotNull('url_api')
            ->where('url_api', '!=', '')
            ->get()
            ->groupBy('url_api');

        foreach ($groupedConfigs as $url => $configs) {
            try {
                $response = Http::get($url);
                Log::info("Código de estado de la respuesta HTTP: " . $response->status());

                if ($response->successful()) {
                    Log::info("Datos recibidos de la API: " . json_encode($response, JSON_PRETTY_PRINT));
                    // Procesar cada configuración del grupo
                    foreach ($configs as $config) {
                        $id = $config->id;
                        //llamamos a procesar el mesaje
                        $this->processMessage($id, $response);
                    }
                } else {
                    Log::error("Error leyendo Modbus API: " . $response->status() . " - " . $response->body());
                }
            } catch (\Exception $e) {
                Log::error("Error completo: " . $e->getMessage() . " en la línea " . $e->getLine());
            }
        }
    ...
    private function getValueFromJson($data, $jsonPath)
{
    ...
    // Verifica si el jsonPath existe directamente en el array
    if (isset($data[$jsonPath])) {
        return $data[$jsonPath]['value'] ?? null;
    } else {
        Log::warning("Clave no encontrada: {$jsonPath}");
    }
    ...
    $newBoxNumber = intval($config->rec_box);
        ...
        try {
            ...
            $lastRep = 0; // Reiniciar el contador de repeticiones
            $dimension_max = 0; //Reiniciar altura de la caja palet
...
//informacion total sobre este producto
