...
class CalculateProductionMonitorOeeController extends Controller
{
    ...
    /**
     * API endpoint para calcular el OEE de producción
     * 
     * @param Request $request
     * @return JsonResponse
     */
    public function calculateOee(Request $request): JsonResponse
    {
        // Permitir que la API continue ejecutándose aunque se pare el comando
        ...
        try {
            ...
            foreach ($monitors as $monitor) {
                try {
                    ...
                    Log::error("[API OEE] Error procesando monitor {$monitor->id}: " . $e->getMessage());
                }
            }
            ...
            return response()->json([
                'success' => true,
                'message' => 'Cálculo de OEE completado',
                'data' => [
                    'processed_monitors' => $processedMonitors,
            ...
            Log::error('[API OEE] Error general: ' . $e->getMessage());
            
            return response()->json([
                'success' => false,
                'message' => 'Error en el cálculo de OEE',
                'error' => $e->getMessage(),
                'timestamp' => Carbon::now()->toDateTimeString()
            ], 500);
        }
    ...
    /**
     * Procesar un monitor específico
     */
    private function processMonitor($monitor)
    {
        $productionLineId = $monitor->production_line_id;

        // Obtener el último shift_history
        ...
        if ($lastShiftHistory && (
            ...
            if ($monitor->sensor_active == 1) {
                $this->processSensors($monitor, $currentOrder, $countSensors, $productionLineId);
            } else {
            }

            if ($monitor->modbus_active == 1) {
                $this->processModbuses($monitor, $productionLineId);
            } else {
            }
        } else {
        }
    ...
    /**
     * Procesar sensores de una línea de producción
     */
    private function processSensors($monitor, $currentOrder, $countSensors, $productionLineId)
    {
        $sensors = Sensor::where('production_line_id', $productionLineId)->get();
        $totalUnitsShift = 0;
        $totalUnitsWeek = 0;

        // Sumar el downtime_count de los sensores
        $totalDowntimeType1Seconds = $sensors->where('sensor_type', 1)->sum('downtime_count');
        $totalDowntimeType1Minutes = floor($totalDowntimeType1Seconds / 60);

        $totalDowntimeType2Seconds = $sensors->where('sensor_type', 2)->sum('downtime_count');
        $totalDowntimeType2Minutes = floor($totalDowntimeType2Seconds / 60);
    ...
    /**
     * Procesar modbuses de una línea de producción
     */
    private function processModbuses($monitor, $productionLineId)
    {
    ...
    /**
     * Helper method para logging (reemplaza $this->info del Command)
     */
    ...
    /**
     * Helper method para logging de errores (reemplaza $this->error del Command)
     */
    ...
    /**
     * Helper method para logging de warnings (reemplaza $this->warn del Command)
     */
